...
test('seeds the bundled remote plugin with runtime dependencies and a registry source', async (t) => {
  ...
  writeJson(join(sourceDir, 'package.json'), {
    name: 'ds-harness-remote', version: '0.4.13', dependencies: { werift: '0.24.4' },
  ...
  assert.match(readFileSync(join(profileDir, 'pnpm-lock.yaml'), 'utf8'), /version: 0\.4\.13/)
...
test('seeds the bundled remote plugin when an older release marked the default as seen', async (t) => {
  ...
  writeJson(join(sourceDir, 'package.json'), {
    name: 'ds-harness-remote', version: '0.4.13',
    dsh: { bundle: { patch: './cordis.patch.yml' } },
  })
  writeFileSync(join(sourceDir, 'index.js'), 'export {}\n')
...
test('upgrades an older exact bundled remote version from the desktop bundle', async (t) => {
  ...
  profileManifest.dependencies['ds-harness-remote'] = '0.4.7'
  ...
  writeJson(join(sourceDir, 'package.json'), {
    name: 'ds-harness-remote', version: '0.4.13', dsh: { bundle: { patch: './cordis.patch.yml' } },
  })
  ...
  assert.equal(plugin.requested, '0.4.13')
  assert.equal(plugin.source, 'npm')
  assert.equal(plugin.version, '0.4.13')
...
test('removes stale legacy remote package names even when their spec is unknown', async (t) => {
  ...
  const nextManifest = JSON.parse(readFileSync(join(profileDir, 'package.json'), 'utf8'))
  assert.equal(nextManifest.dependencies['dsh-remote'], undefined)
  assert.equal(nextManifest.dependencies['ds-harness-remote'], '0.4.13')
  assert.equal(nextManifest.dsh.profile.bundles.includes('dsh-remote'), false)
...
test('updates an npm plugin to the latest registry version and preserves its disabled state', async (t) => {
  ...
  writeJson(profileManifest, {
    name: 'dsh-profile-web',
    private: true,
    dependencies: { 'example-npm-plugin': '0.2.4' },
    dsh: { profile: { bundles: [] } },
  })
  writeJson(join(profileDir, 'node_modules', 'example-npm-plugin', 'package.json'), {
    name: 'example-npm-plugin', version: '0.2.4', dsh: { bundle: { patch: 'cordis.patch.yml' } },
  })
  const pnpmCalls = []
  const runPnpmImpl = async ({ args }) => {
    pnpmCalls.push(args)
    ...
    const manifest = JSON.parse(readFileSync(profileManifest, 'utf8'))
    manifest.dependencies['example-npm-plugin'] = '0.2.5'
    writeJson(profileManifest, manifest)
    writeJson(join(profileDir, 'node_modules', 'example-npm-plugin', 'package.json'), {
      name: 'example-npm-plugin', version: '0.2.5', dsh: { bundle: { patch: 'cordis.patch.yml' } },
    })
    return { output: '' }
  }
  ...
  const catalog = await updatePlugin({
    ...
    fetchImpl: async (url, options) => {
      assert.equal(String(url), 'https://registry.example.test/npm/example-npm-plugin/latest')
  ...
  assert.deepEqual(pnpmCalls, [
    ['config', 'get', 'registry'],
    ['add', '--workspace-root', '--save-prod', '--reporter', 'append-only', 'example-npm-plugin@0.2.5'],
  ])
  assert.equal(catalog.upToDate, false)
...
test('does not downgrade a bundled npm plugin that is newer than the registry latest', async (t) => {
  ...
  writeJson(profileManifest, {
    name: 'dsh-profile-web',
    private: true,
    dependencies: { 'example-npm-plugin': '0.3.0' },
    dsh: { profile: { bundles: ['example-npm-plugin'] } },
  })
  writeJson(join(profileDir, 'node_modules', 'example-npm-plugin', 'package.json'), {
    name: 'example-npm-plugin',
    version: '0.3.0',
